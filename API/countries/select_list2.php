<?php 
	...
	try{
		...
		if($_POST['page_limit'] == "0"){
			$country_data_qry = $conn->prepare("SELECT id, name, code FROM sol_countries WHERE id>0 AND is_deleted='0' ".$search_qry);
		}else{
			$country_data_qry = $conn->prepare("SELECT id, name, code FROM sol_countries WHERE id>0 AND is_deleted='0' ".$search_qry." limit ?,?");
		...
		$sol_countries = $country_data_result->fetch_all(MYSQLI_ASSOC);

		$total_country_qry = $conn->prepare("SELECT id FROM sol_countries WHERE id>0 AND is_deleted='0' ".$search_qry);
