...
include('../../helper/functions.php');
	...
	try{
		...
		$updated_at=$created_at;
				...
				foreach ($ids as $id) 
				{
					$lead_detail_qry = "SELECT * FROM me_provider_ethnicity WHERE id=?  limit 1";
					$lead_data_result = prepared_query($conn, $lead_detail_qry, [$id]);
					$lead_data = $lead_data_result->get_result()->fetch_assoc();
					if($lead_data){
						
						$remove_lead = "UPDATE me_provider_ethnicity SET is_deleted=?, updated_at=? WHERE id=? ";
				...
				$data=$ids;
				$code = 1;
				$message = "provider ethnicity role deleted successfully.";
