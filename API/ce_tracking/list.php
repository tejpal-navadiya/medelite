	...
	try{
		...
		if(isset($headers['Apitoken']) || isset($headers['apitoken'])){
			...
			if($apitoken && $token_id ){
				...
				// {
					// $user_condition=" AND ( ce_tracking.user_id='$token_id' OR ce_tracking.created_by='$token_id') ";
				// }
				...
				$searchQuery = " ";
				// if($searchValue != ''){
				//    $searchQuery = " AND ((ce_tracking.description LIKE :description) OR (m.name LIKE :case_name) OR (m.description LIKE :case_description)";
				//     $searchArray = array( 
				...
				if($searchValue != '')
				{
					$searchQuery .= " AND ((state_content_requirement LIKE '%".$searchValue."%') OR (completed_date LIKE '%".$searchValue."%') OR (provider_approving_body LIKE '%".$searchValue."%') )";
				}

				$filterQuery = " ";
				// if($filterValue != ''){
				// 	$filterQuery = " AND ce_tracking.type = '$filterValue' ";
				// }
				
				// if(isset($_REQUEST['id_case']) && $_REQUEST['id_case']!="")
				// {
				// 	$filterQuery .= " AND ce_tracking.case_id='".$_REQUEST['id_case']."' ";
				// }
				...
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM ce_tracking  
				WHERE is_deleted='0' 
				 ".$filterQuery);
				$stmt->execute();
				$records = $stmt->fetch();
				...
				$stmt->execute();
				$ce_tracking = $stmt->fetchAll();

				$data = array();
				
				foreach($ce_tracking as $row)
				{
					$state_id = $row['state_one'];
					 // Fetch the state name based on the state_id
					 $state_name_query = mysqli_query($conn, "SELECT * FROM me_states WHERE id = '" . $row['state_one'] . "'");
