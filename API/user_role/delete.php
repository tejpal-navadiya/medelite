...
include('../../helper/functions.php');
	...
	try{
		$headers = apache_request_headers();
				...
				foreach ($ids as $id) 
				{
					$lead_detail_qry = "SELECT * FROM me_user_type WHERE id=?  limit 1";
					$lead_data_result = prepared_query($conn, $lead_detail_qry, [$id]);
					$lead_data = $lead_data_result->get_result()->fetch_assoc();
					if($lead_data){
						
						$remove_lead = "UPDATE me_user_type SET is_deleted=? WHERE id=? ";
						$is_deleted = prepared_query($conn, $remove_lead, [1, $id])->affected_rows;
