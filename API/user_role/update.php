	...
	try{
		...
		if(isset($headers['Apitoken']) || isset($headers['apitoken'])){
			...
			if($apitoken && $token_id){
				...
				$ac_data_qry = "SELECT id FROM me_user_type WHERE id=? limit 1";
				...
				if($ac_data_result->num_rows){
					...
					$is_updated = mysqli_query($conn,"UPDATE me_user_type SET name='$name', updated_by='$updated_by', updated_at='$updated_at' WHERE id='$id' ");
