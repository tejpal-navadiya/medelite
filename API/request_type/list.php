	...
	try{
		$headers = apache_request_headers();
			...
			$default_date_format="m/d/Y";
				...
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM me_request_type WHERE  is_deleted='0'   ".$filterQuery);
				$stmt->execute();
				$records = $stmt->fetch();
				$totalRecords = $records['allcount'];

				## Total number of records with filtering
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM me_request_type WHERE  is_deleted='0'   ".$searchQuery." ".$filterQuery);
				...
				$stmt = $pdoconn->prepare("SELECT * FROM me_request_type  WHERE  is_deleted='0'   ".$filterQuery." ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
