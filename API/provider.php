<?php
	...
	function deleteProvider($conn, $firm_id, $updated_by, $ids){
		$ids = json_decode($ids);
		foreach ($ids as $id) {
			$task_detail_qry = "SELECT id FROM me_provider WHERE id=? AND firm_id=? limit 1";
			...
			if($task_data){
				...
				$remove_task = "UPDATE me_provider SET is_deleted=?, updated_by=? WHERE id=? ";
				$is_deleted = prepared_query($conn, $remove_task, [1, $updated_by, $id])->affected_rows;

				// $task_log_data_qry = "SELECT me_provider.*, m.name as matter_name, cb.name as cb_name
				// 							FROM me_provider
				// 							LEFT JOIN matters m on m.id = me_provider.matter_id 
				// 							LEFT JOIN users cb on cb.id = me_provider.created_by 
				// 							WHERE me_provider.id=? limit 1";
				...
				// 	}
				// 	saveLog($conn, $firm_id, $updated_by, "deleted", $id, "me_provider", "", $log_description);
				// }
			}
		}
	}
