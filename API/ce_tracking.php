<?php
	...
	function deleteCetracking($conn,$ids){
		$ids = json_decode($ids);
		foreach ($ids as $id) {
			$task_detail_qry = "SELECT * FROM ce_tracking WHERE id='$id' limit 1";
			...
			if($task_data_result)
			{
				...
				$remove_task = "UPDATE ce_tracking SET is_deleted='1' WHERE id='$id' ";
