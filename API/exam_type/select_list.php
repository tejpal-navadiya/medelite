<?php 
	...
	try{
		...
		if($_POST['page_limit'] == "0"){
			$state_data_qry = $conn->prepare("SELECT id, name FROM me_exam_type WHERE id>0 AND is_deleted='0' ".$search_qry);
		}else{
			$state_data_qry = $conn->prepare("SELECT id, name FROM me_exam_type WHERE id>0 AND is_deleted='0' ".$search_qry." limit ?,?");
			$state_data_qry->bind_param("ii", $page_offset, $page_limit);
		}
		$state_data_qry->execute();
		$state_data_result = $state_data_qry->get_result();
		$me_exam_type = $state_data_result->fetch_all(MYSQLI_ASSOC);

		$total_state_qry = $conn->prepare("SELECT id FROM me_exam_type WHERE id>0 AND is_deleted='0' ".$search_qry);
