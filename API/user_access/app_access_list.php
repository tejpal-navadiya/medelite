	...
	try{
		...
		if(isset($headers['Apitoken']) || isset($headers['apitoken']) || isset($_REQUEST['apitoken']))
		{
			...
			if($apitoken && $token_id && $firm_id){
				...
				$user_data_result = prepared_select($conn, $user_data_qry, [$token_id]);
				$user_data = $user_data_result->fetch_assoc();
				
				if(isset($user_data['access_role']))
				{
					$utype=$user_data['access_role'];
				}
				// $utype=$_POST['utype'];
				...
				$user_access_data = $pa_data_result->fetch_all(MYSQLI_ASSOC);
				...
				foreach($user_access_data as $row){
					...
					$row['pmenu']=$pmenu_name;
				   	...
				   	$user_access_list[] = $row;
				}
				$granted_access_list=array();
				if(count($user_access_list)>0)
				{
					for ($uic=0; $uic < count($user_access_list); $uic++) 
					{ 
						$current_access=$user_access_list[$uic];
						if($current_access['is_access']=="1")
						{
							$cmenu=$current_access['mname'];
							array_push($granted_access_list,$cmenu);
						}
					
					}
					// echo "<script>console.log(".json_encode($granted_access_list).")</script>";

				}
				
				$data=$granted_access_list;
