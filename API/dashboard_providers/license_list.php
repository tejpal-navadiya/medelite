	...
	try{
		...
		if(isset($headers['Apitoken']) || isset($headers['apitoken'])){
			...
			if($apitoken && $token_id && $firm_id){
				...
				// {
					// $user_condition=" AND ( me_license_type.user_id='$token_id' OR me_license_type.created_by='$token_id') ";
				// }
				...
				$searchQuery = " ";
				// if($searchValue != ''){
				//    $searchQuery = " AND ((me_license_type.description LIKE :description) OR (m.name LIKE :case_name) OR (m.description LIKE :case_description)";
				//     $searchArray = array( 
				...
				$filterQuery = " ";
				// if($filterValue != ''){
				// 	$filterQuery = " AND me_license_type.type = '$filterValue' ";
				// }
				
				// if(isset($_REQUEST['id_case']) && $_REQUEST['id_case']!="")
				// {
				// 	$filterQuery .= " AND me_license_type.case_id='".$_REQUEST['id_case']."' ";
				// }
				...
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM me_license_type  
				WHERE is_deleted='0'
				 ".$filterQuery);
				$stmt->execute();
				$records = $stmt->fetch();
				...
				## Fetch records
				$stmt = $pdoconn->prepare("SELECT * FROM me_license_type WHERE is_deleted='0' ".$filterQuery." ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
				...
				$stmt->execute();
				$me_license_type = $stmt->fetchAll();

				$data = array();
				
				foreach($me_license_type as $row)
				{
