	...
	try{
		...
		if(isset($headers['Apitoken']) || isset($headers['apitoken'])){
			...
			if($apitoken && $token_id && $firm_id){
				...
				$columnName = $_POST['order_by'] ?: 'me_provider.id';
				...
				$searchQuery = " ";
				if($searchValue != ''){
				   $searchQuery = " AND (me_provider.description LIKE :description) ";
				    $searchArray = array( 
				...
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM me_provider WHERE is_deleted=0 ".$filterQuery." AND firm_id=".$firm_id);
				$stmt->execute();
				$records = $stmt->fetch();
				$totalRecords = $records['allcount'];

				## Total number of records with filtering
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM me_provider WHERE is_deleted=0 ".$filterQuery." AND firm_id=".$firm_id." ".$searchQuery);
