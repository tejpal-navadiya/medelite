	...
	try{
		...
		if(isset($headers['Apitoken']) || isset($headers['apitoken'])){
			...
			if($apitoken && $token_id && $firm_id){
				...
				// echo "SELECT mid, mtitle FROM admin_menu WHERE mid>'0' AND pmenu='0' ".$search_qry."  limit ?,?";
				$pa_data_qry = $conn->prepare("SELECT mid, mtitle FROM admin_menu WHERE mid>'0' AND pmenu='0' ".$search_qry."  limit ?,?");
...
// 
				$total_pa_qry = $conn->prepare("SELECT mid FROM admin_menu WHERE mid>'0' AND pmenu='0' ".$search_qry." ");
