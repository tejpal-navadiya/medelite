	...
	try{
		...
		if(isset($headers['Apitoken']) || isset($headers['apitoken'])){
			...
			if($apitoken && $token_id && $firm_id){
				...
				$searchQuery = " ";
				if($searchValue != ''){
				   $searchQuery = " AND (me_tasks.verification_request LIKE :description) ";
				    $searchArray = array( 
				        'verification_request'=>"%$searchValue%", 
				    );
				}
				...
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM me_tasks WHERE is_deleted=0 ".$filterQuery." );
				$stmt->execute();
				$records = $stmt->fetch();
				$totalRecords = $records['allcount'];

				## Total number of records with filtering
				$stmt = $pdoconn->prepare("SELECT COUNT(*) AS allcount FROM me_tasks WHERE is_deleted=0 ".$filterQuery." ." ".$searchQuery);
				...
				$stmt = $pdoconn->prepare("SELECT me_tasks.*, m.name as case_name
											...
											WHERE me_tasks.is_deleted='0' ='".$firm_id."' ".$filterQuery." ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
