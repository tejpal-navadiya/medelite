...
$(function() {
    ...
    $('#speciality_subspecialityTable').DataTable({
        'order': [
            [0, 'desc']
        ],
        'processing': true,
        ...
        'ajax': {
            'type': "POST",
            'url': api_url + "speciality_subspeciality/list.php",
            'headers': {
                'apitoken': apitoken
            },
...

            {
                ...
                "render": function(data, type, row, meta) {
                    var btnGroup = '<div class="btn-group">' +
                        ...
                        '<a href="index.php?pid=add_speciality_subspeciality&id=' + row.id + '" class="btn btn-outline-primary btn-sm"><i class="fas fa-edit"></i></a>' +
                        '<button type="button" onclick="delete_speciality_subspeciality_btn(' + row.id + ')" class="btn btn-outline-primary btn-sm"><i class="fas fa-trash-alt"></i></button>' 
                        '</div>';
                    return btnGroup;
                }
            },
...
function reloadFundTable() {
    $('#speciality_subspecialityTable').DataTable().ajax.reload();
    location.reload();
}
...
$("#speciality_subspecialityTable").on('draw.dt', function() {
...
function delete_speciality_subspeciality_btn(id){
  var ids = [];
  ids.push(id);
  delete_speciality_subspeciality(ids);
}
...
function delete_speciality_subspeciality(ids){
  if(ids.length){
    bootbox.confirm({
      ...
      callback: function (result) 
      {
        ...
        if(result)
        {
          ...
          $.ajax({
            type: "POST",
            url: api_url+"speciality_subspeciality/delete.php",
            headers: {
              'apitoken': apitoken
            },
            data: {
