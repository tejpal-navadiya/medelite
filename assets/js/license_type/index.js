...
$(function() {
    ...
    $('#license_typeTable').DataTable({
        ...
        'ajax': {
            'type': "POST",
            'url': api_url + "license_type/list.php",
            'headers': {
                'apitoken': apitoken
            },
...

            {
                ...
                "render": function(data, type, row, meta) {
                    var btnGroup = '<div class="btn-group">' +
                        ...
                        '<a href="index.php?pid=add_license_type&id=' + row.id + '" class="btn btn-outline-primary btn-sm"><i class="fas fa-edit"></i></a>' +
                        '<button type="button" onclick="delete_license_type_btn(' + row.id + ')" class="btn btn-outline-primary btn-sm"><i class="fas fa-trash-alt"></i></button>' 
                        '</div>';
                    return btnGroup;
                }
            },
...
function reloadFundTable() {
    $('#license_typeTable').DataTable().ajax.reload();
    location.reload();
}

$('#select_all_license_type').on('click', function() {
...
function user_roleselected() {
    if ($('.user_roles_selected:checked').length == $('.user_roles_selected').length) {
        $("#select_all_license_type").prop("checked", true);
    } else {
        $("#select_all_license_type").prop("checked", false);
    }
}
...
$('#user_roles_delete').on('click', function() {
    ...
    delete_license_type(ids);

});



$("#license_typeTable").on('draw.dt', function() {
...
function delete_license_type_btn(id){
  var ids = [];
  ids.push(id);
  delete_license_type(ids);
}
...
function delete_license_type(ids){
  if(ids.length){
    bootbox.confirm({
      ...
      callback: function (result) 
      {
        ...
        if(result)
        {
          ...
          $.ajax({
            type: "POST",
            url: api_url+"license_type/delete.php",
            headers: {
              'apitoken': apitoken
            },
            data: {
