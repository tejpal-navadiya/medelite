...
$(function() {
    ...
    $('#method_of_requestTable').DataTable({
        'order': [
            [0, 'desc']
        ],
        'processing': true,
        ...
        'ajax': {
            'type': "POST",
            'url': api_url + "method_of_request/list.php",
            'headers': {
                'apitoken': apitoken
            },
...

            {
                ...
                "render": function(data, type, row, meta) {
                    var btnGroup = '<div class="btn-group">' +
                        ...
                        '<a href="index.php?pid=add_method_of_request&id=' + row.id + '" class="btn btn-outline-primary btn-sm"><i class="fas fa-edit"></i></a>' +
                        '<button type="button" onclick="delete_method_of_request_btn(' + row.id + ')" class="btn btn-outline-primary btn-sm"><i class="fas fa-trash-alt"></i></button>' 
                        '</div>';
                    return btnGroup;
                }
            },
...
function reloadFundTable() {
    $('#method_of_requestTable').DataTable().ajax.reload();
    location.reload();
}

$('#select_all_method_of_request').on('click', function() {
...
function user_roleselected() {
    if ($('.user_roles_selected:checked').length == $('.user_roles_selected').length) {
        $("#select_all_method_of_request").prop("checked", true);
    } else {
        $("#select_all_method_of_request").prop("checked", false);
    }
}
...
$('#user_roles_delete').on('click', function() {
    ...
    delete_method_of_request(ids);

});



$("#method_of_requestTable").on('draw.dt', function() {
...
function delete_method_of_request_btn(id){
  var ids = [];
  ids.push(id);
  delete_method_of_request(ids);
}
...
function delete_method_of_request(ids){
  if(ids.length){
    bootbox.confirm({
      ...
      callback: function (result) 
      {
        ...
        if(result)
        {
          ...
          $.ajax({
            type: "POST",
            url: api_url+"method_of_request/delete.php",
            headers: {
              'apitoken': apitoken
            },
            data: {
