...
$(function() {
    ...
    $('#practice_facility_typeTable').DataTable({
        'order': [
            [0, 'desc']
        ],
        'processing': true,
        ...
        'ajax': {
            'type': "POST",
            'url': api_url + "practice_facility_type/list.php",
            'headers': {
                'apitoken': apitoken
            },
...

            {
                ...
                "render": function(data, type, row, meta) {
                    var btnGroup = '<div class="btn-group">' +
                        ...
                        '<a href="index.php?pid=add_practice_facility_type&id=' + row.id + '" class="btn btn-outline-primary btn-sm"><i class="fas fa-edit"></i></a>' +
                        '<button type="button" onclick="delete_practice_facility_type_btn(' + row.id + ')" class="btn btn-outline-primary btn-sm"><i class="fas fa-trash-alt"></i></button>' 
                        '</div>';
                    return btnGroup;
                }
            },
...
function reloadFundTable() {
    $('#practice_facility_typeTable').DataTable().ajax.reload();
    location.reload();
}

$('#select_all_practice_facility_type').on('click', function() {
...
function user_roleselected() {
    if ($('.user_roles_selected:checked').length == $('.user_roles_selected').length) {
        $("#select_all_practice_facility_type").prop("checked", true);
    } else {
        $("#select_all_practice_facility_type").prop("checked", false);
    }
}
...
$('#user_roles_delete').on('click', function() {
    ...
    delete_practice_facility_type(ids);

});



$("#practice_facility_typeTable").on('draw.dt', function() {
...
function delete_practice_facility_type_btn(id){
  var ids = [];
  ids.push(id);
  delete_practice_facility_type(ids);
}
...
function delete_practice_facility_type(ids){
  if(ids.length){
    bootbox.confirm({
      ...
      callback: function (result) 
      {
        ...
        if(result)
        {
          ...
          $.ajax({
            type: "POST",
            url: api_url+"practice_facility_type/delete.php",
            headers: {
              'apitoken': apitoken
            },
            data: {
