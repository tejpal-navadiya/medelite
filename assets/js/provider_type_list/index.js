...
$(function() {
    ...
    $('#provider_typeTable').DataTable({
        ...
        'ajax': {
            'type': "POST",
            'url': api_url + "provider_type_list/list.php",
            'headers': {
                'apitoken': apitoken
            },
...

            {
                ...
                "render": function(data, type, row, meta) {
                    var btnGroup = '<div class="btn-group">' +
                        ...
                        '<a href="index.php?pid=add_provider_type_list&id=' + row.id + '" class="btn btn-outline-primary btn-sm"><i class="fas fa-edit"></i></a>' +
                        '<button type="button" onclick="delete_provider_type_btn(' + row.id + ')" class="btn btn-outline-primary btn-sm"><i class="fas fa-trash-alt"></i></button>' 
                        '</div>';
                    return btnGroup;
                }
            },
...
function reloadFundTable() {
    $('#provider_typeTable').DataTable().ajax.reload();
    location.reload();
}
...
$("#provider_typeTable").on('draw.dt', function() {
...
function delete_provider_type_btn(id){
  var ids = [];
  ids.push(id);
  delete_provider_type(ids);
}
...
function delete_provider_type(ids){
  if(ids.length){
    bootbox.confirm({
      ...
      callback: function (result) 
      {
        ...
        if(result)
        {
          ...
          $.ajax({
            type: "POST",
            url: api_url+"provider_type_list/delete.php",
            headers: {
              'apitoken': apitoken
            },
            data: {
