...
$(function() {
    ...
    $('#cert_statusTable').DataTable({
        ...
        'ajax': {
            'type': "POST",
            'url': api_url + "cert_status/list.php",
            'headers': {
                'apitoken': apitoken
            },
...

            {
                ...
                "render": function(data, type, row, meta) {
                    var btnGroup = '<div class="btn-group">' +
                        ...
                        '<a href="index.php?pid=add_cert_status&id=' + row.id + '" class="btn btn-outline-primary btn-sm"><i class="fas fa-edit"></i></a>' +
                        '<button type="button" onclick="delete_cert_status_btn(' + row.id + ')" class="btn btn-outline-primary btn-sm"><i class="fas fa-trash-alt"></i></button>' 
                        '</div>';
                    return btnGroup;
                }
            },
...
function reloadFundTable() {
    $('#cert_statusTable').DataTable().ajax.reload();
    location.reload();
}

$('#select_all_cert_status').on('click', function() {
...
function user_roleselected() {
    if ($('.user_roles_selected:checked').length == $('.user_roles_selected').length) {
        $("#select_all_cert_status").prop("checked", true);
    } else {
        $("#select_all_cert_status").prop("checked", false);
    }
}
...
$('#user_roles_delete').on('click', function() {
    ...
    delete_cert_status(ids);

});



$("#cert_statusTable").on('draw.dt', function() {
...
function delete_cert_status_btn(id){
  var ids = [];
  ids.push(id);
  delete_cert_status(ids);
}
...
function delete_cert_status(ids){
  if(ids.length){
    bootbox.confirm({
      ...
      callback: function (result) 
      {
        ...
        if(result)
        {
          ...
          $.ajax({
            type: "POST",
            url: api_url+"cert_status/delete.php",
            headers: {
              'apitoken': apitoken
            },
            data: {
